...
func _on_GameMap_cell_clicked(clicked_map_position):
    ...
    if active_state == state.MOVE:
        ...
        print('current: %s, desired: %s' % [current_rotation, desired_rotation])

        if not is_equal_approx(current_rotation, desired_rotation):
            tween.interpolate_property($Sprite, "rotation",
                    current_rotation, desired_rotation, 0.5,
        ...
        tween.interpolate_property(self, "position",
                position, game_map.map_to_world(clicked_map_position), 0.5,
                Tween.TRANS_QUINT, Tween.EASE_IN_OUT)
        tween.start()
        yield(tween, 'tween_completed')
        ...
        $Node2D/Missile.global_position = game_map.map_to_world(clicked_map_position) # TODO: kinda gross?
